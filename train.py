...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            # Get model and loss
            pred, index = MODEL.get_model(pointclouds_pl, is_training_pl, bn_decay=bn_decay)
        ...
        for epoch in range(MAX_EPOCH):
            ...
            if acc > acc_record and epoch > 20:
                acc_record = acc
                saver.save(sess, os.path.join('log/best_model', 'model.ckpt'))

            if epoch > 20:
                print('current best acc:', acc_record)

            # Save the variables to disk.
            if epoch % 10 == 0:
                save_path = saver.save(sess, os.path.join(LOG_DIR, 'model.ckpt'))
                log_string("Model saved in file: %s" % save_path)
        log_string("best acc: %s" % acc_record)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for fn in range(len(TRAIN_FILES)):
        log_string('----' + str(fn) + '-----')
        current_data, current_label = provider.loadDataFile(Data_Path+TRAIN_FILES[train_file_idxs[fn]])
        ...
        for batch_idx in range(num_batches):
            ...
            summary, step, _, loss_val, pred_val, index = sess.run([ops['merged'], ops['step'],
                ops['train_op'], ops['loss'], ops['pred'], ops['index']], feed_dict=feed_dict)
